...
| Screen | What it does |
|---|---|
| **Dashboard** | Portfolio overview: total exposure, weighted average interest rate, outstanding vs. collected amounts, collection rate, overdue amount/count, and active loan/borrower counts (`utils/portfolioMetrics.ts`). |
| **Due Payments** | Every overdue, not-fully-settled installment across all borrowers, sorted most-overdue first, with one-tap payment recording and a jump to the borrower's profile (`utils/duePayments.ts`). |
...
components/                   Screens and reusable UI widgets
...
- **`context/StorageContext.tsx`** is the single source of truth. Every
...
- **`services/sheetsSync.ts`** is a thin HTTP client for one Google Apps
...
| Date picker behaves differently on iOS vs Android | Expected — Android shows a native inline dialog, iOS opens a bottom-sheet modal with Done/Cancel (`components/DatePicker.tsx`). |
