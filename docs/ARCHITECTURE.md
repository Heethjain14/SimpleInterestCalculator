...
navigation/screens.ts           Screen union type + sidebar nav item metadata
components/                     All UI screens and reusable widgets
context/StorageContext.tsx      Single source of truth for borrower/loan state
hooks/useStorage.ts             Deprecated alias for useStorageContext()
hooks/useNotifications.ts       No-op stub (reminders not implemented yet)
services/sheetsSync.ts          HTTP client for the Google Apps Script webapp
...
### Navigation (`navigation/screens.ts`, `App.tsx`, `components/Sidebar.tsx`)
...
back into `App.tsx` via `onSelect`. `navigation/screens.ts` is the single
...
`SimpleInterestCalculator`, `EmiCalculator`, `BorrowerList`) read from
`useStorageContext()` (or the `useStorage()` alias) and render one of the
five screens. `BorrowerList` internally swaps between the list, `BorrowerForm`
(create/edit), and `BorrowerDetail` (profile + loans) without a router —
plain `useState`.

Reusable widgets (`DatePicker`, `ShareResultCard`, `RefreshButton`,
`PaymentRecorder`) are shared across screens and hold no borrower state of
their own; they take data in via props and report changes via callbacks.
...
### State management (`context/StorageContext.tsx`)
...
`hooks/useStorage.ts` is a thin, deprecated wrapper around
`useStorageContext()` kept for components that were written before the
context existed; both are used interchangeably across `components/`.
...
### Services / sync layer (`services/sheetsSync.ts`)
...
directly via GET, and nothing in `components/`, `context/`, or `hooks/`
references `localhost`, a proxy port, or this file. Treat it as an optional,
