...
`services/sheetsSync.ts` (and `testing.js`) send every request as a **GET**,
...
`services/sheetsSync.ts`) rather than as one big payload.
...
`type` values used by `services/sheetsSync.ts` today — your `handleRequest`
...
- Failure: `{ ok: false, reason: 'api_error', message: '...' }`. The
  `RefreshResult` type in `context/StorageContext.tsx` also recognizes
